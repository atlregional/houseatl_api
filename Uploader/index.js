...
const date = new Date();
const todaysDate = `${date.getMonth() + 1
  ...
  try {
    ...
    for await (const item of dataArr) {
      ...
      if (Subsidy.start_date) {
        ...
        console.log('Property:', Property)
        console.log('Subsidy:', Subsidy)
        console.log('Geocoded:', geocodedObj)

        if (!error) {
          await handleCollectionsInsert(
            new ObjectId(userId),
        ...
        console.log('No start date:', Property);
      }
    }
